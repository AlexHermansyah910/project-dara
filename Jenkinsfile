...
sudo docker run -d -p ${DB_PORT_CONTAINER} --name ${DB_CONTAINER_NAME} --restart unless-stopped -e MARIADB_ROOT_PASSWORD=12345678 -e MARIADB_DATABASE=${DB_NAME} --network ${DB_NETWORK_NAME} -v ${DB_VOLUME_NAME}:/var/lib/mysql docker.io/mariadb
...
sudo docker exec -it ${CONTAINER_NAME} php artisan migrate
sudo docker exec -it ${CONTAINER_NAME} php artisan db:seed RoleSeeder
sudo docker exec -it ${CONTAINER_NAME} php artisan db:seed UserSeeder
EOF
                        '''
                    }
                }
